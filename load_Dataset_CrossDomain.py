        ...
        for i, datatype in enumerate(DataType):
            ...
            with open(path.join(Source_Final_Data_Path, 'Label.txt'), 'r') as f:
                for line in f.readlines():
                    ...
                    self.Source_Label[split_line[0]] = str(int(split_line[1]) + classes_num * i)
...
class PD_PLDataModule_CrossDomain(LightningDataModule):
    def __init__(self, Dataset_Path, Source_Domain, Target_Domain, DataType, Num_Workers, Pin_Memory, Batch_Size,
                 Use_Spurious_Label, Classes_num,
    ...
    def setup(self, stage: str):
        if stage == 'fit':
            self.train_data = PD_Dataset_CrossDomain(self.Dataset_Path, self.Source_Domain, self.Target_Domain,
                                                     self.DataType, 'train', self.Use_Spurious_Label, self.Classes_num,
                                                     self.Cache,
                                                     self.Target_Source_rate,
                                                     self.Spurious_Label_Update, self.Init_conf_threshold)

            self.val_data_source = PD_Dataset(self.Dataset_Path, self.Source_Domain, self.DataType, 'val',
                                              self.Classes_num, self.Cache)
            self.val_data_target = PD_Dataset(self.Dataset_Path, self.Target_Domain, self.DataType, 'val',
                                              self.Classes_num, self.Cache)
        if stage == 'test':
            self.test_data_source = PD_Dataset(self.Dataset_Path, self.Source_Domain, self.DataType, 'test',
                                               self.Classes_num, self.Cache)
            self.test_data_target = PD_Dataset(self.Dataset_Path, self.Target_Domain, self.DataType, 'test',
                                               self.Classes_num, self.Cache)
